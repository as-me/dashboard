(function (angular) {
    ...
    // weavecore.log = new weavecore.SessionStateLog(WeaveAPI.globalHashMap);
    //weavecore.SessionStateLog.debug = true;
    ...
    module.factory('slidingPuzzle', function () {
        ...
        function SlidingPuzzle(rows, cols) {
             ...
             */
            this.move = function () {
                var srow = this.rowNumber.value;
                var scol = this.colNumber.value;
            ...
            this.clickListener = function (srow, scol) {
                this.rowNumber.value = srow;
                this.colNumber.value = scol;

            }

            this.rowNumber = WeaveAPI.globalHashMap.requestObject("rowNumber", weavecore.LinkableNumber, false);
            this.colNumber = WeaveAPI.globalHashMap.requestObject("colNumber", weavecore.LinkableNumber, false);

            this.rowNumber.addImmediateCallback(this, this.move.bind(this));
            this.colNumber.addImmediateCallback(this, this.move.bind(this));
            weavecore.log.clearHistory();
    ...
    module.directive('slidingPuzzle', function (slidingPuzzle) {
        return {
            ...
            template: '<table class="sliding-puzzle" ng-class="{\'puzzle-solved\': puzzle.isSolved()}">' +
                ...
                '<td ng-repeat="($col, tile) in row" ng-click="puzzle.clickListener($row, $col)" ng-style="tile.style" ng-class="{\'puzzle-empty\': tile.empty}" title="{{tile.id}}"></td>' +
                '</tr>' +
                '</table>',
            scope: {
                size: '@',
