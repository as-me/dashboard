...
var Servlet = {
    ...
    queryAPIService: function (url, resultHandler, queryId) {
            ...
            if (!isServletCall) {
                ...
                try {
                    if (isError) {
                        ...
                        if (resultHandler)
                            resultHandler(jsonResponse, queryId);
                        if (promise) // which calls the function attached with then
     ...
     * @return A Promise.
     */
    bulkQueryAPIService: function (url, method, queryIdToParams, resultsHandler) {
        var batch = [];
        for (var queryId in queryIdToParams) {
            var m = typeof method === 'string' ? method : method[queryId];
            batch.push({
